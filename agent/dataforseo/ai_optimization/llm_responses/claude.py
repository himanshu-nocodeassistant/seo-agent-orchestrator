...
class ClaudeResponses(DataForSEOClient):

    def models(self) -> list[dict]:
        """
        GET /ai_optimization/claude/llm_responses/models

        Lists Claude model names available for llm_responses calls, and
        whether each supports web search and task_post (queued) mode.

        Result dict contains:
            model_name, web_search_supported, task_post_supported
        """
        data = self._get("ai_optimization/claude/llm_responses/models")
    ...
    def live(self, tasks: list[dict]) -> list[dict]:
        ...
        Each task dict may contain:
            ...
            model_name                    (str)   e.g. "claude-opus-4-0", default set by DataForSEO
            system_message                (str)
        ...
        """
        data = self._post("ai_optimization/claude/llm_responses/live", tasks)
        return self._extract_results(data)

    def task_post(self, tasks: list[dict]) -> list[dict]:
        """
        POST /ai_optimization/claude/llm_responses/task_post

        Queues a Claude prompt for asynchronous processing. Takes the
        ...
        """
        data = self._post("ai_optimization/claude/llm_responses/task_post", tasks)
        return self._extract_results(data)

    def tasks_ready(self) -> list[dict]:
        """
        GET /ai_optimization/claude/llm_responses/tasks_ready
        ...
        """
        data = self._get("ai_optimization/claude/llm_responses/tasks_ready")
        return self._extract_results(data)

    def task_get(self, task_id: str) -> list[dict]:
        """
        GET /ai_optimization/claude/llm_responses/task_get/{id}
        ...
        Each result dict contains:
            ...
            datetime, items [{type, sections: [{type, text, annotations}]}]
        """
        data = self._get(f"ai_optimization/claude/llm_responses/task_get/{task_id}")
        return self._extract_results(data)
