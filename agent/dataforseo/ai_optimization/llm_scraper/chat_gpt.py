...
class ChatGPTScraper(DataForSEOClient):

    def live_advanced(self, tasks: list[dict]) -> list[dict]:
        """
        POST /ai_optimization/chat_gpt/llm_scraper/live/advanced

        Scrapes a live ChatGPT web response for a keyword and returns
        ...
        Each result dict contains:
            ...
            item_types, items_count,
            items [{type: "chat_gpt_text", rank_group, rank_absolute, markdown, sources}]
        """
        data = self._post("ai_optimization/chat_gpt/llm_scraper/live/advanced", tasks)
        return self._extract_results(data)

    def live_html(self, tasks: list[dict]) -> list[dict]:
        """
        POST /ai_optimization/chat_gpt/llm_scraper/live/html
        ...
        """
        data = self._post("ai_optimization/chat_gpt/llm_scraper/live/html", tasks)
        return self._extract_results(data)
    ...
    def task_post(self, tasks: list[dict]) -> list[dict]:
        ...
        Queues a ChatGPT scrape for asynchronous processing. Takes the
        same task dict fields as live_advanced()/live_html(). Poll
        tasks_ready() then fetch with task_get_advanced()/task_get_html().
        """
        data = self._post("ai_optimization/chat_gpt/llm_scraper/task_post", tasks)
        return self._extract_results(data)

    def tasks_ready(self) -> list[dict]:
        """
        GET /ai_optimization/chat_gpt/llm_scraper/tasks_ready
        ...
        """
        data = self._get("ai_optimization/chat_gpt/llm_scraper/tasks_ready")
        return self._extract_results(data)

    def task_get_advanced(self, task_id: str) -> list[dict]:
        """
        GET /ai_optimization/chat_gpt/llm_scraper/task_get/advanced/{id}
        ...
        Each result dict contains:
            ...
            items [{type: "chat_gpt_text", rank_group, rank_absolute, markdown, sources}]
        """
        data = self._get(f"ai_optimization/chat_gpt/llm_scraper/task_get/advanced/{task_id}")
        return self._extract_results(data)

    def task_get_html(self, task_id: str) -> list[dict]:
        """
        GET /ai_optimization/chat_gpt/llm_scraper/task_get/html/{id}
        ...
        Each result dict contains:
            ...
            items [{page, date, html}]
        """
        data = self._get(f"ai_optimization/chat_gpt/llm_scraper/task_get/html/{task_id}")
        return self._extract_results(data)

    def locations(self) -> list[dict]:
        """
        GET /ai_optimization/chat_gpt/llm_scraper/locations

        Lists locations supported by the ChatGPT scraper.

        Result dict contains:
            location_code, location_name, location_code_parent,
            country_iso_code, location_type
        """
        data = self._get("ai_optimization/chat_gpt/llm_scraper/locations")
        return self._extract_results(data)

    def languages(self) -> list[dict]:
        """
        GET /ai_optimization/chat_gpt/llm_scraper/languages

        Lists languages supported by the ChatGPT scraper.

        Result dict contains:
            language_name, language_code
        """
        data = self._get("ai_optimization/chat_gpt/llm_scraper/languages")
        return self._extract_results(data)
